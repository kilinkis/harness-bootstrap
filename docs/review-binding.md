...
Four completed bootstrap features recorded their actual final approvals under older filenames. The shared resolver in `scripts/final-review.ts` reads the pinned designations from `harness.bootstrap-history.json`:
...
Preserve the historical reports. Do not change their recorded hashes to bypass a failure. For a fresh queue, omit `harness.bootstrap-history.json` and `tests/harness/bootstrap-history.test.ts`. Keep the reusable validators and canonical-review contracts unchanged.

## Historical bootstrap evidence

Only the original completed definitions of TASK-001, TASK-002, and TASK-004 are exempt from retrospective implementation reports, final reviews, history entries, and binding. `harness.bootstrap-history.json` pins each complete queue definition with SHA-256; `scripts/bootstrap-history.ts` verifies it. Object key order does not affect identity. Changed titles, criteria, status, or other fields remove the exception. Reusing one of these IDs does not exempt replacement work.
...
Preserve the three historical queue definitions and existing history. Do not fabricate old reports or update the pinned hashes to exempt new work. When adopting a fresh queue without this bootstrap history, omit the history data file and its dedicated test file. Missing data grants no exemptions. Malformed or unreadable supplied data produces `BOOTSTRAP_HISTORY_INVALID`; partial pins are not used. Keep the new-work, reference, and evidence rejection contracts.
