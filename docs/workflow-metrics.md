...
The harness records small workflow events as JSON Lines. Each line is one version 1 event. The runtime contract and `workflow-metrics.schema.json` define the accepted fields.
...
```bash
HARNESS_METRICS_FILE=artifacts/workflow-metrics.jsonl ./scripts/verify.sh
```
...
## Automatic gate events

The public `pnpm run feedback`, `pnpm run verify:docs`, and `./scripts/verify.sh` entry points record the fast, docs, and full gates. Each event contains the schema version, gate name, outcome, start time, wall time, and exit code. Start time uses the canonical UTC form `YYYY-MM-DDTHH:mm:ss.sssZ`. A passed event has exit code 0. A failed event has an exit code from 1 through 255.
