...
Dependency maintenance is the only additional no-active-feature exception. It permits pnpm lockfile changes, dependency or package-manager declarations in `package.json`, and the pnpm setup version in the verification workflow. It does not permit scripts, source, arbitrary manifest fields, other workflow changes, queue evidence, or unknown files. The full gate and required remote review remain mandatory.
